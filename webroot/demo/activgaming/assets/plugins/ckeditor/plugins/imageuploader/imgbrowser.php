...
{
	echo '<meta http-equiv="refresh" content="0; url=http://activ-gaming.com/?page=user&view=login&name=Se%20connecter" />';
	exit();
}
...
<?php if($file_style == "block") { ?>
    <div class="fullWidthFileDiv">
        <p>Pour utiliser l'image dans l'article, cliquez sur votre image et appuyez sur "Utiliser" ou copiez l'URL.</p>
        
        <button 
            onclick="uploadImg();" 
            style="background-color: #2C3E50; color: #fafafa; padding: 8px; border: 1px solid #2C3E50;">
            Uploader une image
        </button>
    </div>
<?php } elseif($file_style == "list") { ?>
...
<div id="imageFullSreen" class="lightbox popout">
    <div class="buttonBar">
        ...
        <button class="headerBtn greenBtn" id="imgActionUse" onclick="#" class="imgActionP"><img src="img/cd-icon-use.png" class="headerIcon"> <?php if(!empty($_SESSION['is_admin'])): ?><?= $buttons3; ?><?php endif; ?></button>
    </div><br><br>
...
<div id="uploadImgDiv" class="lightbox popout">
    ...
    <form action="imgupload.php" method="post" enctype="multipart/form-data" id="uploadImgForm" onsubmit="return checkUpload();">
        ...
        <p>Après avoir effectué l'upload de votre image faîtes un clique droit dessus pour récupérer l'URL.</p>
        <p><button class="headerBtn greenBtn" name="submit" onclick="$('#uploadImgForm').submit();"><img src="img/cd-icon-upload.png" class="headerIcon"> <?php echo $buttons7; ?></button></p>
        <br/>
    </form>
</div>
